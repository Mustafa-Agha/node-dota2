...
const SteamGameCoordinator = require("node-steam-game-coordinator");
...
Dota2.Dota2Client = function Dota2Client(steamClient, debug, debugMore) {
  ...
  this._gc = new SteamGameCoordinator(steamClient, DOTA_APP_ID);
  steam.SteamGameCoordinator = this._gc;
  ...
  this._sendClientHello = function () {
    ...
    if (!self._gc) {
      ...
      self._gc.send(self._protoBufHeader, Dota2.schema.lookupType("CMsgClientHello").encode(payload).finish());
    }
...
Dota2.Dota2Client.prototype.sendToGC = function (type, payload, handler, callback) {
  ...
  this._gc.send(
    this._protoBufHeader, // protobuf header, same for all messages
...
handlers[Dota2.schema.lookupEnum("EGCBaseClientMsg").values.k_EMsgGCClientWelcome] = function clientWelcomeHandler(
  message
) {
...
handlers[Dota2.schema.lookupEnum("EGCBaseClientMsg").values.k_EMsgGCClientConnectionStatus] =
  function gcClientConnectionStatus(message) {
